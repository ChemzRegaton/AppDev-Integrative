...
function AdminBookManage() {
    ...
    const handleDeleteBook = async (bookId) => {
        if (window.confirm(`Are you sure you want to delete book with ID: ${bookId}?`)) {
            try {
                await axios.delete(`http://appdev-integrative-28.onrender.com:8000/api/library/books/${bookId}/`);
    ...
    const handleBorrowBook = async (bookId) => {
        ...
        try {
            const response = await axios.post(
                `http://appdev-integrative-28.onrender.com:8000/api/library/books/${bookId}/borrow/`,
